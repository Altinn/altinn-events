...
namespace Altinn.Platform.Events.Services
{
    ...
    public class OutboundService : IOutboundService
    {
        ...
        public async Task PostOutbound(CloudEvent cloudEvent)
        {
            ...
            {
                // matching subscription for external events will be handled in altinn/altinn-events#107
                return; 
            }

            // GetSourceFilter method should only be used for events generated by apps.
            string simplifiedSource = GetSourceFilter(cloudEvent.Source);
            if (string.IsNullOrEmpty(simplifiedSource))
            {
                simplifiedSource = cloudEvent.Source.ToString();
            }

            List<Subscription> subscriptions = await GetSubscriptions(simplifiedSource, cloudEvent.Subject, cloudEvent.Type);
