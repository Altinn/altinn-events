...
namespace Altinn.Platform.Events.Services
{
    ...
    public class TraceLogService(ITraceLogRepository traceLogRepository, ILogger<ITraceLogService> logger) : ITraceLogService
    {
        ...
        public async Task<string> CreateLogEntryDuplicateIdempotencyKeySkipped(CloudEvent cloudEvent, Guid? idempotencyKey)
        {
            ...
            {
                _logger.LogError(exception, "Error creating trace log entry for registered event with existing idempotency key: {CloudEventId} with message: {Message}", cloudEvent.Id, exception.Message);
