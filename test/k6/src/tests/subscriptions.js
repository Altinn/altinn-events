...
export async function setup() {
    ...
    let incorrectScopeToken = await getAltinnTokenForOrg("altinn:serviceowner");
...
function TC03_PostExistingSubscription(data) {
    ...
    let subscription = JSON.parse(response.body);
    let success = check(response, {
        "03 - POST existing subscription. Status is 201": (r) =>
            r.status === 201,
...
function TC06_DeleteSubscription(data, subscriptionId) {
    let response = subscriptionsApi.deleteSubscription(
        subscriptionId,
        data.orgToken
    );
    let success = check(response, {
...
function TC07_PostSubscriptionExternalEventSource(data) {
    ...
    let subscription = JSON.parse(response.body);
    let success = check(response, {
        "07 - POST subscription for external event source. Status is 201": (
            r
        ) => r.status === 201,
...
export default function runTests(data) {
    try {
        if (data.runFullTestSet) {
            const appSubscriptionId =
                TC01_PostNewSubscriptionForAppEventSource(data);
